...
void MainWindow::on_peth_clicked()
{
    int p = tree.maxDepth();
    QString peth = QString::number(p);
    ui->textEdit_ans->setText(peth);
}


void MainWindow::on_pushButton_balance_clicked()
{
    tree.balanceTree(); // Вызов метода балансировки дерева
    model->clear();
    displayTree(model->invisibleRootItem(), tree.getRoot()); // Перестроение отображения дерева
    ui->treeView->expandAll();
}
