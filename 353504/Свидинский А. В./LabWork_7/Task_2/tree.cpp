...
int Tree::maxDepthUtil(TreeNode *node) {
    if (node == nullptr) {
        return 0; // Если узел пуст, его глубина 0
    } else {
        // Рекурсивно вычисляем глубину левого и правого поддеревьев
        int leftDepth = maxDepthUtil(node->left);
        int rightDepth = maxDepthUtil(node->right);

        // Максимальная глубина - это наибольшая из двух глубин поддеревьев + 1 для текущего узла
