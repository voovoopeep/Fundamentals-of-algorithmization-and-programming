...
QStandardItem *createItemForNode(AVLTree<QString>::Node *root, const QString &position = "")
{
    ...
    QString label = position.isEmpty()
        ? "Корень(↓" + QString::number(root->height) + "↓) - " + "<" + QString::number(root->key) + ">"
                        : "(↓" + QString::number(root->height) + "↓)" + position + " - <"
                              + QString::number(root->key) + ">";
    QStandardItem *item = new QStandardItem(label);
    QStandardItem *leftChild = createItemForNode(root->left, "Левое");
    QStandardItem *rightChild = createItemForNode(root->right, "Правое");
