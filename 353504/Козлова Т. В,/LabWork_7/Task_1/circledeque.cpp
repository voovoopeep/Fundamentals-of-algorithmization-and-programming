...
template<typename T>
void CircleDeque<T>::push_back(T value)
{
...
template<typename T>
void CircleDeque<T>::push_front(T value)
{
    if (_size == _capasity * BLOCK_SIZE
...
template<typename T>
T &CircleDeque<T>::operator[](size_t index)
{
    index = index % _size;
...
template<typename T>
const T &CircleDeque<T>::operator[](size_t index) const
{
    index = index % _size;
...
template<typename T>
const T &CircleDeque<T>::at(size_t index) const
{
    index = index % _size;
...
template<typename T>
size_t CircleDeque<T>::size() const
{
    return _size;
}

template<typename T>
bool CircleDeque<T>::empty() const
{
    return _size == 0;
}

template<typename T>
void CircleDeque<T>::pop_back()
{
    if (empty()) {
        return;
    }
...
template<typename T>
DequeIterator<T> CircleDeque<T>::begin()
{
    return DequeIterator(this, 0);
}

template<typename T>
DequeIterator<T> CircleDeque<T>::end()
{
    return DequeIterator(this, _size);
}

template class CircleDeque<QString>;
