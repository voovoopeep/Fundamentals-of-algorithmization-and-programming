...
Six_star::Six_star(QPointF point_first)
{
    point1 = point_first;
    point2 = point1;
}

QString Six_star::Perim()
{
    return QString::fromStdString(std::to_string(perim));
}

QString Six_star::Plohchad()
{
    return QString::fromStdString(std::to_string(ploshad));
}

QString Six_star::Zenter_mass()
{
    return QString::fromStdString(std::to_string(zentr_mass_x) + ';' + std::to_string(zentr_mass_y));
}

void Six_star::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    Q_UNUSED(*option)
    Q_UNUSED(*widget)
    ...
    const int kolvo_yglov = 6;
    const int kolvo_treug = 12;

    const int last_point = 5;
