...
class BST {
 ...
 private:
  void insertInTree(int key, QString info, Data*& tree) {
    if (tree == nullptr)
      tree = new Data(info, key);
    else if (key <= tree->key)
      insertInTree(key, info, tree->leftSon);
    else
      insertInTree(key, info, tree->rightSon);
  }
  ...
  QString findInTree(int key, Data* tree) {
    ...
    if (key > tree->key)
      return findInTree(key, tree->rightSon);
    else
      return findInTree(key, tree->leftSon);
  }
 ...
 public:
  ...
  BST(QVector<QPair<int, QString>> info) {
    for (QPair<int, QString> el : info) insert(el.first, el.second);
  }

  void insert(int key, QString info) { insertInTree(key, info, root); }
  QString find(int key) { return findInTree(key, root); }
  void remove(int key) {
    Data* a = nullptr;
  ...
  int calcLeaves() { return calcLeavesOfTree(root); }
};
