...
private:
    void insertInTree(T key, Data *&tree)
    {
        if (tree == nullptr)
            ...
            insertInTree(key, tree->leftSon);
        else
            insertInTree(key, tree->rightSon);
    }
        ...
        if (key < tree->key) {
            ...
            if (tree->leftSon == nullptr && tree->rightSon == nullptr) {
                ...
                if (parent == nullptr) {
                    ...
                    if (side == -1)
                        parent->leftSon = tree->rightSon;
                    else if (side == 1)
                        parent->rightSon = tree->rightSon;
                    delete temp;
                }
                ...
                if (parent == nullptr) {
                    ...
                    if (side == -1)
                        parent->leftSon = tree->leftSon;
                    else if (side == 1)
                        parent->rightSon = tree->leftSon;
                    delete temp;
                }
            } else {
    ...
    Data *findLCA(Data *node, T n1, T n2)
    {
        ...
        if (node->key > n1 && node->key > n2)
            return findLCA(node->leftSon, n1, n2);

        if (node->key < n1 && node->key < n2)
            return findLCA(node->rightSon, n1, n2);
...
public:
    ...
    void insert(T key) { insertInTree(key, root); }
    ...
    Data *findNode(T key, Data *tree)
    {
        ...
        if (tree->key == key)
            return tree;
        if (key > tree->key)
            return findNode(key, tree->rightSon);
        else
            return findNode(key, tree->leftSon);
    }

    void remove(T key) { removeFromTree(key, root); }
    ...
    void balanceTree() { balanceTreeHelper(root); }

    void insertSubtree(Data *subtree, T parentKey)
    {
        Data *parentNode = findNode(parentKey, root);
        if (parentNode) {
            if (parentNode->leftSon == nullptr)
                parentNode->leftSon = subtree;
            else if (parentNode->rightSon == nullptr)
                parentNode->rightSon = subtree;
        }
    }
    ...
    void insertBranch(Data *branch, T parentKey) { insertSubtree(branch, parentKey); }

    void removeBranch(T key) { removeSubtree(key); }
    ...
    Data *findLCA(T n1, T n2) { return findLCA(root, n1, n2); }
};
