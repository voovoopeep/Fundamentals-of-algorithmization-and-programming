...
#include <QtMath>
FivePointedStar::FivePointedStar() = default;

auto FivePointedStar::Area() -> qreal
{
    return this -> getScale() * this -> getScale() * (M_PI * radius1 * radius1 + (M_PI * radius2 * radius2 - M_PI * radius1 * radius1) / 3);
}

auto FivePointedStar::Perimeter() -> qreal
{
    return this -> getScale() * _multiplier * (radius1 + radius2);
}

void FivePointedStar::paint(QPainter *painter,
                            ...
                            QWidget * /*unused*/)
{
    ...
    for(int i = 0; i < _multiplier; ++i)
    {
        ...
        starPoints[i * 2 + 1] = QPointF(static_cast<qreal>(-radius2 / 2
                                                           * sin(2 * M_PI / 5 * i + M_PI / _multiplier)),
                                        static_cast<qreal>(-radius2 / 2
                                                           * cos(2 * M_PI / 5 * i + M_PI / _multiplier)));
    }
