...
private:
    ...
    {
        // Продолжаем перемещать элемент вниз, пока есть хотя бы один дочерний элемент
        while (2 * index + 1 < heap->size())
        {
            // Определяем индекс максимального дочернего элемента
            ...
            // Если текущий элемент больше или равен максимальному дочернему, то он на своем месте
            if (heap->at(index) >= heap->at(maxChild))
            {
                break;
            }

            // Меняем местами текущий элемент и максимальный дочерний
            swap(index, maxChild);
            index = maxChild;
        }
    }
...
public:
    ...
    void drawTree(QGraphicsScene* &scene, int x, int y, int level, int index)
    {
        ...
        // Определяем расстояние между узлами в зависимости от размера дерева
        float tab = 100;
        if (size > 20)
        {
            tab *= (float)size / 20.0;
        ...
        // Рисуем левого потомка, если он существует
        ...
        // Рисуем правого потомка, если он существует
