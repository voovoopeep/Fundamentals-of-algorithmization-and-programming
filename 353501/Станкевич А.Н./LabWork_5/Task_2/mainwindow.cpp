...
void MainWindow::on_bnt_memcpy_clicked()
{
    ...
    qstrncpy(str1, byteArray1.constData(), n);
...
{
    QString str1 = ui->ln_str1->text();

    char* src = new char[str1.length() + 1];
    strcpy(src, str1.toStdString().c_str());

    char dest[100] = {0};

    strcpy(dest, src);

    ui->ln_ans->setText(QString::fromLatin1(dest));
...
void MainWindow::on_bnt_strncpy_clicked()
{
    ...
    QString("С помощью метода strncpy n символов было скопировано из String1 в QLineEdit"));
}
...
void MainWindow::on_bnt_strcat_clicked()
{
    ...
    int totalLength = byteArray1.size() + byteArray2.size() + 1;


    char* str = new char[totalLength];


    qstrncpy(str, byteArray1.constData(), byteArray1.size());
