...
public:
    ...
    void remove(int index)
    {
        ...
        {
            throw std::runtime_error("index out of border");
        }
    ...
    void changePriority(int index, T element)
    {
        if (index < 0 || index >= heap->size())
        {
            throw std::runtime_error("index ot of border");
        }
        T old = heap->at(index);
    ...
    void drawTree(QGraphicsScene* &scene, int x, int y, int level, int index)
    {
        ...
        {
            scene->addLine(x+12, y+20, x-offset+25, y+40);
