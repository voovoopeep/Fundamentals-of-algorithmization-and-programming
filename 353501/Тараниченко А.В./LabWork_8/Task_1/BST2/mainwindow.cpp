...
void MainWindow::fillTreeWidgetFromBST(QTreeWidgetItem *item, BSTree<QString>::Data *node)
{
    if (node == nullptr)
        return;
...
void MainWindow::on_insertSubtreeButton_clicked()
{
    ...
    tree.insertSubtree(BSTree<QString>(a).root, ui->infoLineEdit->text());
    displayTree();
}
