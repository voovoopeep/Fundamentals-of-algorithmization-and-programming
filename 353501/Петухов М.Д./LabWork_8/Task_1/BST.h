...
class BinarySearchTree {
    ...
    public:
        ...
        Node* deleteNodeNoBalance(Node* root, long k) {
            if (root == nullptr)
                return root;

            // If the key to be deleted is smaller than the root's key, then it lies in the left subtree
            if (k < root->key) {
                root->left = deleteNodeNoBalance(root->left, k);
            }
            // If the key to be deleted is greater than the root's key, then it lies in the right subtree
            else if (k > root->key) {
                root->right = deleteNodeNoBalance(root->right, k);
            }
            else{
                if (root->left == nullptr) {
                    Node* temp = root->right;
                ...
                root->key = tmp->key;
                root->value = tmp->value;
                root->right = deleteNodeNoBalance(root->right, tmp->key);
            }
        ...
        void destroySubTree(Node *node) {
            if (node != nullptr) {
                ...
                deleteNodeNoBalance(root, node->key);
            }
        }

        void destroyRightBranch(Node* node){
            // Node* temp = node->right->left;
            // subFuncToDelRightBranch(node);
            // node->right = temp;

            destroySubTree(node->right);
        }

        // void subFuncToDelRightBranch(Node* node){
        //     if(node != nullptr && node->right != nullptr){
        //         subFuncToDelRightBranch(node->right);
        //         deleteNodeNoBalance(root, node->right->key);
        //     }
        // }

        void destroyLeftBranch(Node* node){
            // Node* temp = node->left->right;
            // subFuncDelLeftBranch(node);
            // node->left = temp;
            destroySubTree(node->left);
        }

        // void subFuncDelLeftBranch(Node* node){
        //     if(node != nullptr && node->left != nullptr){
        //         subFuncDelLeftBranch(node->left);
        //         deleteNodeNoBalance(root, node->left->key);
        //     }
        // }
        ...
        Node* insertOtherTree(const BinarySearchTree& anotherTree){
            if(anotherTree.root != nullptr){
                insertNodeFromAnotherTree(anotherTree.root);
            }
            return this->root;
        }

        void insertNodeFromAnotherTree(Node* node){
            this->root = insert(this->root, node->key, node->value);
            if (node->right)
                insertNodeFromAnotherTree(node->right);
            if (node->left)
                insertNodeFromAnotherTree(node->left);
        }

        Node* findCommonKey(Node* root, Node* node1, Node* node2) {
            if (root == nullptr || root == node1 || root == node2) {
                return root;
            }

            Node* left = findCommonKey(root->left, node1, node2);
            Node* right = findCommonKey(root->right, node1, node2);

            // Если один из узлов найден в левом поддереве, а другой в правом, текущий узел - общий предок
