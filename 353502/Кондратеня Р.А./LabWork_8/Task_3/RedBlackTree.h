...
template <typename KeyT, typename ValT> class Map: public RedBlackTree<KeyT, ValT> {
    using RedBlackTreee = RedBlackTree<KeyT, ValT>;
public:
    ValT& operator[](const KeyT& key) {
        auto t = RedBlackTreee::findNode(key);
        if(t != nullptr) {
            return t->val;
        };
        return RedBlackTreee::insert(key, ValT())->val;
    };
...
template <typename ValT> class Set: public RedBlackTree<ValT, char> {
    using RedBlackTreee = RedBlackTree<ValT, char>;
public:
    void insert(ValT&& val) {
        RedBlackTreee::insert(val, 'a');
    }
};
