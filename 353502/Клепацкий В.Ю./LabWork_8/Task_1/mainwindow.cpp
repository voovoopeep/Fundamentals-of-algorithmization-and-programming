...
{
    ui->setupUi(this);
    ui->bnt_remove_subtree->setVisible(false);
    ui->lineEditText_2->setVisible(false);
    ui->spb_val_2->setVisible(false);
    ui->bnt_add_item_2->setVisible(false);
    ui->treeView_2->setVisible(false);
    ui->el_1->setVisible(false);
    ui->el_2->setVisible(false);
    ui->bnt_add_subtree->setVisible(false);
    ui->bnt_remove_subtree->setVisible(false);
    ui->bnt_cnt_of_list->setVisible(false);
...
void MainWindow::on_bnt_cnt_of_list_clicked()
{
    ...
    if (!d1 || !d2) {
        ...
        QMessageBox::information(this,
                                 "Ключ наименьшего общего предка                     ",
                                 QString::number(key->key));
    }
}
