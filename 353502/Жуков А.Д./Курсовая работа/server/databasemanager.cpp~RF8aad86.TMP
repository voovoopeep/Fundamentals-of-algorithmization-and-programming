...
void DatabaseManager::addMessage(const QString &message, const QString &fromUser, const QString &toUser, const QString &isGroup, const QString &type, const QString &file)
{
...
QString DatabaseManager::getUserChatNames(int userId)
{
    ...
    query->prepare(R"(
        SELECT *
        FROM chatsList
        WHERE chat_name LIKE :user_id;
    )");
    query->bindValue(":user_id", "%" + uName + "%");
